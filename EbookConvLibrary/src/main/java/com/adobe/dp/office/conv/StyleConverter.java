...
class StyleConverter {
	...
	private static void setIfNotPresent(InlineRule rule, String name, CSSValue value) {
		Object val = rule.get(name);
		if (val == null || val instanceof CSSImpliedValue)
			rule.set(name, value);
	}

	private CSSValueList convertBorderSide(BorderSide side) {
	...
	boolean convertLabelToProperty(NumberingLabel label, InlineRule rule) {
	...
	StylingResult getLabelRule(InlineRule paragraphRule, NumberingLabel label, float emScale) {
	...
	private Hashtable cascadeWordProperties(BaseProperties prop) {
	...
	StylingResult convertTableStylingRule(TableProperties prop, float emScale) {
		Hashtable wprop = cascadeWordProperties(prop);
		StylingResult result = new StylingResult();
	...
	StylingResult convertTableCellStylingRule(TableCellProperties prop, float emScale,
