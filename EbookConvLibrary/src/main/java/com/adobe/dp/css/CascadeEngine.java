...
public class CascadeEngine {
	...
	private CascadeResult result;

	private final Vector matcherLists = new Vector();

	private final Hashtable classMap = new Hashtable();

	private final Hashtable tagMap = new Hashtable();
	...
	static class CascadeValue extends CSSValue {
		...
		CascadeValue(CSSValue value, int specificity, int importance, int order) {
			this.value = value;
	...
	static class MatcherList {
		final Vector matchers = new Vector();
	...
	static class MatcherRule {
		final ElementMatcher matcher;

		final SelectorRule rule;

		final int order;

		MatcherRule(ElementMatcher matcher, SelectorRule rule, int order) {
			super();
	...
	public static void main(String[] args) {
		try {
			...
			reader.setContentHandler(new ContentHandler() {
				...
				public void startElement(String ns, String local, String qname, Attributes attrs) {
